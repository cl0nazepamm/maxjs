...
def _candidate_web_roots():
    env_root = os.environ.get("MAXJS_WEB_DIR", "")
    if env_root:
        yield os.path.normpath(env_root)
    yield _repo_web_root()
    yield os.path.normpath(os.path.join(os.getcwd(), "web"))
...
def _resolve_web_root(scene):
    ...
    if custom and os.path.isdir(custom):
        return custom
    for root in _candidate_web_roots():
        if root and os.path.isdir(root):
            return root
    return _repo_web_root()


def _export(context, *, adaptive_geometry=True, camera_override=None):
    scene = context.scene
