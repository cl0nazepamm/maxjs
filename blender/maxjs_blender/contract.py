...
# Same hash helper as src/maxjs_geometry_sync.h. The seed intentionally matches
# the repo implementation, even though it differs from the textbook FNV offset.
FNV1A_SEED = 1469598103934665603
FNV1A_PRIME = 1099511628211
_U64_MASK = 0xFFFFFFFFFFFFFFFF


def hash_fnv1a(data, seed=FNV1A_SEED):
    """Hash bytes with max.js's optimized FNV-1a variant."""
    if isinstance(data, str):
        data = data.encode("utf-16le")
    p = memoryview(bytes(data)).cast("B")
    n = len(p)
    h = int(seed) & _U64_MASK
    i = 0

    if n >= 64:
        h0 = h
        h1 = h ^ 0x9E3779B97F4A7C15
        h2 = h ^ 0xC2B2AE3D27D4EB4F
        h3 = h ^ 0x165667B19E3779F9
        while i + 31 < n:
            a = int.from_bytes(p[i:i + 8], "little")
            b = int.from_bytes(p[i + 8:i + 16], "little")
            c = int.from_bytes(p[i + 16:i + 24], "little")
            d = int.from_bytes(p[i + 24:i + 32], "little")
            h0 = ((h0 ^ a) * FNV1A_PRIME) & _U64_MASK
            h1 = ((h1 ^ b) * FNV1A_PRIME) & _U64_MASK
            h2 = ((h2 ^ c) * FNV1A_PRIME) & _U64_MASK
            h3 = ((h3 ^ d) * FNV1A_PRIME) & _U64_MASK
            i += 32
        h = ((h0 ^ h1) * FNV1A_PRIME) & _U64_MASK
        h = ((h ^ h2) * FNV1A_PRIME) & _U64_MASK
        h = ((h ^ h3) * FNV1A_PRIME) & _U64_MASK

    if n - i >= 16:
        while i + 15 < n:
            lo = int.from_bytes(p[i:i + 8], "little")
            hi = int.from_bytes(p[i + 8:i + 16], "little")
            h = ((h ^ lo) * FNV1A_PRIME) & _U64_MASK
            h = ((h ^ hi) * FNV1A_PRIME) & _U64_MASK
            i += 16

    while i < n:
        h = ((h ^ int(p[i])) * FNV1A_PRIME) & _U64_MASK
        i += 1
    return h
