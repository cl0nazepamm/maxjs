...
def _append_geo(buf, mesh):
    ...
    Block order v→i→uv→n and adaptive element types exactly mirror
...
class _MaterialLibrary:
    ...
    def intern(self, mat):
        ...
        key = json.dumps(mat, sort_keys=True, separators=(",", ":"))
        hit = self._cache.get(key)
        if hit is not None:
            return hit
        mid = len(self.entries) + 1
        self.entries.append({"id": mid, "hash": (hash(key) & 0x7FFFFFFFFFFFFFFF), "mat": mat})
        self._cache[key] = mid
        return mid
