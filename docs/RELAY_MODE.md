...
- In 3ds Max, choose **Tools > RELAY**. Every viewer boot starts OFF. The relay
  producer module, host observer, frame encoder, timers, and network transport
  are loaded only after that explicit toggle.
...
`enable()`, `disable()`, and `toggle()` return promises because the relay
producer is loaded on demand. Advanced integrations may also use `emit` and
`subscribe`.
Persisted `maxjs.liveRelayUrl` and `maxjs.liveRelayStream` values configure the
producer, but are not even read until Relay is enabled and never auto-enable
it. Disabling Relay unregisters the host observer and tears the producer down.
