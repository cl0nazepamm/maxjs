...
3) Call to "itrees->IForestRenderBegin(t)" . It prepares the object for rendering. If you have some pre-process rendering phase, make the call from there.

4) Call to "GetRenderMesh". It rebuilds the object properly for rendering.
	 The returned mesh will hold the geometry for billboards, which are not instantiable. If you want to provide support for billboards, render this mesh as a standard object.
...
5) Generate the array of unique meshes and get a pointer to it.
	...
	This lets you implement some features, as other Animation modes than "Disabled" and "Follow", which are the only available when instances are referenced to scene INodes.
...
6) Generate the array of instances and get a pointer to it:
	...
	The number of instances will be stored in "numInstances". If the functions fail for some reason, or there are not instantiable items, it will return NULL.
...
8) Once frame is rendered, call to "itrees->IForestRenderEnd(t)". This function sets the object for the viewport, clearing the rendering data.
...
It lets you gather all data before rendering, and use it later. Usually this method is more suitable for GPU and Standalone renderers.
...
There is a function to gather the data, and other to release it:
...
MaxSDK::Array<unsigned int> elmIdxByFace;					This array lets you get the index of the Element which belongs each face of the mesh sample. 
