...
function applyStandardSurface( material, inputs, log, nodeName ) {
	...
	// max.js r186 compatibility: exported opacity can be float or color3.
	// element(float, 0) emits invalid WGSL/GLSL; conversion handles both types.
	const opacityNode = hasNodeValue( inputs.opacity ) ? float( inputs.opacity ) : undefined;
